({
    ...
    /**checkAllQuotasFull: function(component, event, helper) {
        var respondentList = component.get("v.respondentList");
        for(var i=0; i<respondentList.length; i++){
            var allFull = true;
            var quotas = respondentList[i].lstRespQuotas;
            for(var j=0; j<quotas.length; j++){
                var quota = quotas[j];
                var isFull = false;
                for(var k=0; k<component.get("v.TestedGoals").length; k++){
                    var goal = component.get("v.TestedGoals")[k];
                    if(goal.condition === quota.key){
                        if(goal.isFull){
                            isFull = true;
                        }
                    }
            ...
            respondentList[i].allFull = allFull;
        }
        component.set("v.respondentList", respondentList);
    },*/
    ...
    getSessionRecordsDisplay : function(component, event){
        ...
        action.setCallback(this, function(response){
            ...
            if(component.isValid() && state === "SUCCESS"){
                ...
                this.keepSortData(component, event);
                this.toggleSpinner(component, event);
                //this.checkAllQuotasFull(component, event, that);
            }             
        });
        $A.enqueueAction(action);
        if(refreshT == '5'){
    ...
    getLatestDashboardMT : function(component, event){
        //debugger;
        var action = component.get("c.updateTrackerStats");
        action.setParams({masterTestId : component.get("v.MasterTestId"), sessionId : null});
        action.setCallback(this, function(response){
            //debugger;
            var state = response.getState();
            if(component.isValid() && state === "SUCCESS"){
                component.set("v.DashboardMT", response.getReturnValue());
            }             
        });
        $A.enqueueAction(action);
    },
    /**getTestedGoals : function(component, event){
        //debugger;
        var action = component.get("c.updateTestedGoals");
        action.setParams({masterTestId : component.get("v.MasterTestId")});
        action.setCallback(this, function(response){
            //debugger;
            var state = response.getState();
            if(component.isValid() && state === "SUCCESS"){
                component.set("v.TestedGoals", response.getReturnValue());
            }             
        });
        $A.enqueueAction(action);
    },*/
    ...
    clearMessageDelayed : function(component, event){
        window.setTimeout(
            $A.getCallback(function(){
    ...
    keepSortData : function(component, event) {
        //debugger;
        ...
        component.set("v.compare1", compare1);
        component.set("v.compare2", compare2);
        this.runSort(component, event);
    },
    ...
    runSort : function(component, event) {
        //debugger;
        var fldName = component.get("v.selectedColSort");
        var rlist = component.get("v.respondentList");
        var compare1 = component.get("v.compare1");
        var compare2 = component.get("v.compare2");
