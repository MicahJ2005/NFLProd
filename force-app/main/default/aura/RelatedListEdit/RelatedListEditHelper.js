({
    ...
    handleLookupModal : function(component, recordId, relationshipName) {
        ...
        var showLookupModal = !$A.util.isEmpty(component.get("v.selectedRows"));
    ...
    handleEditLookup : function(component, event){
            ...
            for(var i = 0; i < records.length; i++){
                if(records[i].Id == parms.recordId){
                    lookupUpdateMap[parms.recordId] = {
                        ...
                        attributes : {type:component.get("v.childSObjectName")}
                    };
        ...
        if(parms.applyAll){
            ...
            if(!$A.util.isEmpty(selectedRows)){
                for(var i=0; i < selectedRows.length; i++){
                    ...
                    lookupUpdateMap[selectedRow.Id] = {
                        Id : selectedRow.Id,
                        attributes : {type:component.get("v.childSObjectName")}
                    };
                    lookupUpdateMap[selectedRow.Id][parms.relationshipName]=parms.newParentId;
                }
            }
        }
        ...
        debugger;
        if(!$A.util.isEmpty(lookupUpdateMap)){
            
            var recordsToUpdate = [];
            var keys = Object.keys(lookupUpdateMap);
            for(var i=0; i < keys.length; i++){
                var key = keys[i];
                recordsToUpdate.push(lookupUpdateMap[key]);
            }
            
            this.handleAction(
                component,
                {
                    recordString : JSON.stringify(recordsToUpdate)
                },
                "c.updateRecords",
